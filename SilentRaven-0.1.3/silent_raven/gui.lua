...
function gui.render()
    ...
    if rewards_ok and rewards then
        ...
        if src == 'cloud' then
            ...
            render_menu_header(
                'Catalog: embedded fallback (run Updater.bat manually, then Reload Catalog)')
        end
    end
    ...
    gui.elements.reload_catalog_toggle:render('Reload Catalog (local)',
        'Reload the local cache catalog without restarting. Run Updater.bat manually first if you want to fetch an update. The checkbox clears after one reload.')
