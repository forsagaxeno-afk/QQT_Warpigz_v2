...
--      on AND the player is standing in Skov_Temis
...
--          SilentRavenPlugin.pause(caller) / .resume(caller)
...
local gui      = require 'silent_raven.gui'
...
local external = require 'silent_raven.external'
...
-- Debounce the user-requested local catalog reload.
...
local function maybe_consume_external_trigger(now)
    ...
    if (not with_tp) and tracker.last_zone_handled == cur_zone then
        ...
        tracker.finish('skipped_latched')
        return
    end
...
local function handle_manual_keybind(now)
    ...
    if tracker.running or tracker.external_trigger or tracker.managed_by or tracker.paused then
        log.info('manual trigger ignored: another owner is active')
        return
    end
    ...
    if not whispers.player_ready() or not whispers.current_zone() then
        log.info('manual trigger deferred: player or world unavailable')
        return
    end
...
local function handle_reload_catalog(now)
    ...
    if cur and not _last_reload_state then
        ...
        last_reload_t = now
        log.info('Reload Catalog: reading local catalog...')
...
local function main_pulse()
    ...
    tracker.observe_zone(whispers.current_zone())
