...
utils.is_cced = function (local_player)
    ...
    for _, buff in pairs(buffs) do
        if debuffs[tonumber(buff.name_hash)] then
            return true
        end
    end
    return false
...
-- Avoid formatting per-rule/per-cast diagnostics unless DEBUG is enabled.
utils.debug_log = function (format, ...)
    if settings.log_level < 2 then return end
