...
- A target or long path left by another plugin is replaced only when a different caller claims a new goal with `set_target`, `navigate_long_path` or `try_traversal_route`, or when the owner calls `release`. `move`, `update` and `resume` from a different caller never drop it (a companion that drives Batmobile under its own name would otherwise ping-pong the goal); Batmobile logs `[batmobile] move by X: goal of Y kept` at most every 30 s.
- `reset(caller)` also restores the default explorer priority (`direction`). A world change or teleport resets explorer, trap and traversal state automatically. If you come back within 300 s to the same world, within 100 u of the point where you left it or teleported away (for example pit -> town for an Alfred trip -> the same pit through the portal), the explorer map you had there (frontiers, visited cells, backtrack) is restored. Only the most recently left map is kept. Targets, trap and traversal state are still reset. `reset(caller)` discards the cached map.
...
- Trap detection (`is_trapped`, `is_giving_up`, trap-escape climbs) runs while Batmobile is unpaused. While paused, it runs only when the caller's goal is more than 25 u away and the caller is actually driving toward it: its own long route is active, or the player moved at least 2 u in the last second. A paused hold or fight (Maiden, pyre, boss) never trips it.
