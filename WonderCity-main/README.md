...
- Chest completion: our own interaction followed by a stable non-interactable state, or disappearance with new loot (1 s), or a stable disappearance of the interacted chest (5 s). A chest that stays interactable eight seconds after our interaction counts as opened (the host can keep an opened chest flagged interactable). A chest that is not interactable before our first click counts as already opened only if it does not unlock: within 10 s when there is positive evidence of an opening (our own earlier click on this floor's chest, also before an Alfred round trip; a burst of at least two new non-obol items within 5 m of the chest; or a boss kill observed, alive then dead, after the chest was first seen), otherwise within 60 s, because a chest whose boss is outside the actor stream may still be locked. A boss corpse seen together with the chest, or a kill before the chest was first seen, is not evidence. The timer never runs while a boss is alive. Each wait logs one `[WonderCity:chest]` line with the chest key, its interactable state, `boss_kill_time`, the observed kill, the last boss seen and the evidence, so a live log shows which case occurred. A reward chest that was seen and then vanished from complete actor scans for 20 s near its position counts as opened elsewhere. A temporary empty or failed actor list is never evidence. Time spent yielding to Alfred does not count toward these windows.
