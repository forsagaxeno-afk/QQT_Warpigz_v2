...
**Current release: v2.2.0.** Every version is published on the [Releases page](https://github.com/forsagaxeno-afk/QQT_Warpigz_v2/releases) with an installable package. Project release numbering started with v2.0.0. Plugin folder names and upstream history remain separate from the bundle version.
...
| --- | --- | --- |
| `WarPigs-1.0.0` | Master orchestrator and town handoffs | 1.1.2 |
...
| `AlfredTheButler-WarPigz` | Town services (Alfred): mythic uniques, charms and seals | 1.0.0 |
| `TristramLoop` | Standalone Uber Tristram / Whimsyshire loop (not driven by WarPigs) | 1.0.0 |
...
4. **Alfred is now bundled** (`AlfredTheButler-WarPigz`): disable or remove your old Alfred folder (both use the same `AlfredTheButlerPlugin` API and settings label). Keep your separately installed **Looter** and **combat/Orbwalker** plugins; they are not bundled or replaced. Configure town, loot rules, combat, and activity settings before enabling automation.
