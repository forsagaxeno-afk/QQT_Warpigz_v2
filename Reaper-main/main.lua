...
on_update(function()
    ...
    if rotation.is_done() or finishing then
        ...
        if zone == settings.town_zone then
            ...
            local cb = not rotation.failed and run_once_callback or nil
            stop()
...
on_render(function()
    ...
    local x, y = 20, 60
    graphics.text_2d("=== REAPER  v1.9.1  by Magoogle ===", vec2:new(x, y), 14, color_orange(255))
    y = y + 20
...
ReaperPlugin = {
    ...
    -- false if busy, boss_id is unknown, or run_type is unsupported.
    run_boss = function(boss_id, run_type)
        if gui.elements.main_toggle:get() then return false end
    ...
    -- on_complete() is called after Reaper reaches town and stops,
    ...
    --
    -- Returns false while busy or for an unknown boss/unsupported run type.
    --
...
console.print("=============================================")
console.print("  Reaper  v1.9.1  by Magoogle  - Loaded")
