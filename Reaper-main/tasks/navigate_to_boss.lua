...
local function cache_variants(boss_id)
    local variants = {}
...
-- Load recorded paths in Reaper's bootstrap context, before an orchestrator
-- can change the active require context or generic package.loaded entries.
-- Runtime navigation keeps the same nearest-variant/waypoint selection.
for _, boss in ipairs(enums.boss_zones) do
    cache_variants(boss.id)
end

local function load_variants(boss_id)
    return cached_variants[boss_id] or {}
end
...
local function pick_best_path(variants)
    ...
    for _, pts in ipairs(variants) do
        if pts[1] then
            local d = pp:dist_to(pathwalker.point_position(pts[1]))
...
local function find_nearest_path_index(pts, player_pos)
    ...
    for i, raw in ipairs(pts) do
        local pos = pathwalker.point_position(raw)
        if pos then
...
local function get_altar_approach_target(boss)
    ...
    if path and #path > 0 then
        return pathwalker.point_position(path[#path]), "path endpoint"
    end
...
function task.Execute()
    ...
    if nav.state == STATE.IDLE then
        ...
        if in_target_zone(boss) then
            ...
            if BatmobilePlugin then
                navigation_owner.claim(); BatmobilePlugin.reset(plugin_label)
    ...
    if nav.state == STATE.MAP_NAV then
        ...
        if not map_nav.is_active() then
            ...
            if nav.attempts >= nav.max_attempts then
                ...
                rotation.advance("Boss teleport failed repeatedly")
                tracker.reset_run()
                reset_nav(); return
            end
            map_nav.start(boss.id, boss.zone_prefix)
    ...
    if nav.state == STATE.PATHWALKING then
        ...
        local use_batmobile = BatmobilePlugin and (settings.use_batmobile or nav.fallback_to_batmobile)
        if use_batmobile then
            ...
            if pathwalker.is_walking or pathwalker.is_path_completed() then
            ...
            if ok then
                ...
                if nav.long_path_retries >= TRAVERSAL_TRIGGER then
                    if nav.trav_attempts >= MAX_TRAVERSAL_ATTEMPTS then
                        ...
                        rotation.advance("Traversal attempts exhausted")
                        tracker.reset_run()
                        reset_nav()
                        return
                    end
        ...
        if not pathwalker.is_walking and not pathwalker.is_path_completed() then
            local variants = load_variants(boss.id)
            local path = pick_best_path(variants)
            if path then
            ...
            elseif BatmobilePlugin then
                ...
                rotation.advance("No recorded path or navigation fallback")
                tracker.reset_run()
                reset_nav()
                return
            end
        ...
        if pathwalker.is_path_completed() then
            ...
            if not utils.get_altar() then
                rotation.advance("Recorded path ended without an altar")
                tracker.reset_run()
            end
