...
function base.new_task()
    ...
    -- WarPigz: compare without the "(caller) " prefix. The prefix depends on
    -- tracker.external_caller at the time of the check, so a task whose status
    -- was set before a caller arrived (or the initial 'Idle') matched nothing
    -- and fell straight through to FAILED when the player already stood at
    -- the NPC ("Failed to stash" on the first external trigger).
    function task.check_status(status)
        local current = tostring(task.status)
        if current == status then return true end
        local stripped = current:match('^%b() (.*)$')
