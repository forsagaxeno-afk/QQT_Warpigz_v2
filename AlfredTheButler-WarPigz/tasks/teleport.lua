...
task.Execute = function ()
    if tracker.teleport and
        ...
        not (tracker.stash_done or tracker.stash_failed)
    then
        task.retry = 0
        -- Let Looter finish picking up (read-only, bounded): the old fork
        -- wrote LooteerPlugin.setSettings('looting', false) and waited forever.
        local now = get_time_since_inject()
        if utils.looter_busy() then
            task.looter_wait_since = task.looter_wait_since or now
            if now - task.looter_wait_since < LOOTER_WAIT_MAX then
