...
function explorerlite:reset_exploration()
    ...
    current_path = {}
    path_index = 1
    -- WarPigz: was an accidental global write (shared _G); the lite explorer has no modes
...
-- WarPigz: the on_update hook (called world.get_current_world() on a global
-- `world` and the undefined find_target() when stuck) and the on_render debug
-- text drawn at every path point every frame were removed, as in
-- HelltideRevamped's copy of this explorer.
